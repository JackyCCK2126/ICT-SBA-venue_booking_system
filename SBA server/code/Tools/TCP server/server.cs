...
namespace Net
{
    class Server
	{
		...
		private static int MaxByteReceived = 4048;
		...
		{//connect
			...
			{
				//loopreceive should automaticaly and async.ly disconnect the client If there is error on receive (due to client disconnect)
			...
			else
			{
				...
				{
					Console.WriteLine("Client disconnected very quickly after connecting!\n" +
						"(maybe caused by local error that is not seems to be related to server)");
				}
				Console.WriteLine("Server users: " + clientSockets.Count);
			}
			serverSocket.BeginAccept(LoopAccept,null);
		}
